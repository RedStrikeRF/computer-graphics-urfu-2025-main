...
public static class Figures
{
    ...
    public static void DrawSphere(float stepU = 0.1f, float stepV = 0.05f, PrimitiveType drawType = PrimitiveType.Points)
    {
        GL.Begin(drawType);
        GL.Color3(Color.LightBlue);
        float size = 70f;

        for (float u = 0; u <= 2 * MathF.PI; u += stepU) // широта
        {
            for (float v = -MathF.PI / 2; v <= MathF.PI / 2; v += stepV) // долгота
            {
                float x = MathF.Cos(u) * MathF.Cos(v);
                float y = MathF.Sin(u) * MathF.Cos(v);
                float z = MathF.Sin(v);
